...
const SEAT_BACK_CROP = 8; // original 16×32 avatars tuck their legs behind the chair
...
interface CharacterOptions {
  ...
  /** Restricts idle paths only; directed visits may use the full map. */
  roamBounds?: { minX: number; maxX: number; minY: number; maxY: number };
...
export class Character {
  ...
  constructor(options: CharacterOptions) {
    ...
    const bounds = options.roamBounds;
    this.roamMap = bounds ? {
      width: this.mapRenderer.width, height: this.mapRenderer.height,
      isWalkable: (x, y) => x >= bounds.minX && x <= bounds.maxX && y >= bounds.minY && y <= bounds.maxY && this.mapRenderer.isWalkable(x, y),
    } : this.mapRenderer;
  ...
  moveTo(tile: { x: number; y: number }, walkable: Walkable = this.mapRenderer): void {
    const path = findPath(walkable, this.getTilePosition(), tile);
  ...
  /** Cancel a directed activity, including callbacks and temporary FX. */
  stopActivity(clearCup = true): void {
    this.arrivalCallback = null;
    this.cheerT = -1; this.confetti = [];
    this.stopWatering(); this.stopSmoking();
    if (clearCup) this.setCarryingCup(false);
    this.setIdle();
  }

  /** Roam the office between tasks. Picks random walkable tiles and strolls
   *  to them until the agent is given work again. */
  startWandering(restAtDesk = true): void {
    if ((this.idleLoop || !restAtDesk) && this.wandering) return; // already roaming
    // (Re)enter the idle loop at its linger phase, then begin roaming.
    this.idleLoop = restAtDesk;
  ...
  private updateWander(dt: number): void {
    ...
    for (let attempt = 0; attempt < 14; attempt++) {
      ...
      if ((tx !== cur.x || ty !== cur.y) && this.roamMap.isWalkable(tx, ty)) {
        const wasWandering = this.wandering;
        this.moveTo({ x: tx, y: ty }, this.roamMap);   // moveTo() leaves state='walk'

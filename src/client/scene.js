...
import { createPixelActivities } from './pixel-activities.js';
...
import { CHARACTERS, characterFrames, SEATS } from './characters.js';
...
import { reconcileSeats } from '../shared.js';
import { syncMotion } from './motion.js';
import { LEAD_BOUNDS, TEAM_BOUNDS } from '../art/pixel-office.mjs';

/** A self-contained renderer. The caller owns its lifetime and data. */
export async function createOfficeScene(element, onSelect, onError) {
  const app = new Application();
  const atlases = [];
  let frames = [];
  const characters = new Map();
  let observer, activities, effects;
  let seats = new Map();
  let destroyed = false;
  ...
  try {
    ...
    activities=createPixelActivities({characters,map,world,effects});

import { createActivityEffects } from '../activity-effects.js';
import { createTeamActivities } from './activities.js';
import { createOfficeProps } from './props.js';
import * as THREE from 'three';
import { OrbitControls } from 'three/addons/controls/OrbitControls.js';
import { makeTeamOffice } from './model.js';
import { layoutNameplates } from './nameplates.js';
import { assignTeamSeats, STATIONS } from './layout.js';
import { createTeamMotion, updateTeamMotion, poseTeamCharacter, statusMotion } from './motion.js';

export async function createTeamOfficeScene(element, onSelect, onError) {
  ...
  let renderer, model, controls, observer, activities, effects, props, stageGeo, stageMat, frame, last = 0, active = false, destroyed = false;
  let members = [], seats = new Map(), labelFor = value => value, selected = null;
  const motions = STATIONS.map((_, i) => createTeamMotion(i));
  const plates = new Map(), stems = new Map();
  ...
  function hit(event) {
    ...
    let object = hits[0].object; while (object && object.userData.seat === undefined) object = object.parent;
    return object?.userData.seat ?? null;
  }
  const down = event => { pointerStart = [event.clientX,event.clientY]; };
  const move = event => { renderer.domElement.style.cursor = hit(event) === null ? 'grab' : 'pointer'; };
  const up = event => {
  ...
  const lost = event => { if (!destroyed) { event.preventDefault(); setActive(false); onError(new Error('WebGL context lost')); } };
  function draw(now) {
    ...
    model.characters.forEach((rig,index) => {
      ...
      if (!rig.avatar.visible) { rig.ring.visible = false; return; }
  ...
  function destroy() {
    ...
    if (renderer) for (const [name,fn] of [['pointerdown',down],['pointerup',up],['pointermove',move],['keydown',key],['webglcontextlost',lost]]) renderer.domElement.removeEventListener(name,fn);
    activities?.dispose(); effects?.destroy(); props?.destroy(); model?.dispose(); stageGeo?.dispose(); stageMat?.dispose();
    scene.traverse(item=>{if(item.isLight && item.shadow) item.shadow.dispose();});
  ...
  try {
    ...
    for (const [name,fn] of [['pointerdown',down],['pointerup',up],['pointermove',move],['keydown',key],['webglcontextlost',lost]]) canvas.addEventListener(name,fn);
    ...
    activities = createTeamActivities({ motions, rigs:model.characters, seats:()=>seats, members:()=>members.filter(m=>seats.has(m.id)), effects, props });

import { CHAIR, CHAIR_ROUTE, STATIONS, roamPoints } from './layout.js';
export function statusMotion(status) { return status === 'running' ? 'work' : status === 'idle' || status === 'inactive' ? 'walk' : 'stand'; }
export function createTeamMotion(index) {
...
function same(a, b) { return Math.hypot(a.x - b.x, a.z - b.z) < .025; }
export function updateTeamMotion(state, mode, dt) {
...
export function poseTeamCharacter(rig, state) {

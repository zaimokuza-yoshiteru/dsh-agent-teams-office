...
export function createKit() {
  const geometries = new Map(), materials = new Map(), merged = new Set();
  function geometry(key, make) { if (!geometries.has(key)) geometries.set(key, make()); return geometries.get(key); }
  function material(color) { if (!materials.has(color)) materials.set(color, new THREE.MeshStandardMaterial({ color, roughness: 0.78 })); return materials.get(color); }
  function mesh(parent, geo, color, at) {
  ...
  function box(parent, size, at, color, round = .035) {
  ...
  function cylinder(parent, radius, height, at, color, bottom = radius) {
  ...
  function sphere(parent, size, at, color) {
    const item = mesh(parent, geometry('sphere', () => new THREE.SphereGeometry(1, 12, 8)), color, at); item.scale.set(...size); return item;
  }
  function group(parent, at = [0, 0, 0]) { const item = new THREE.Group(); item.position.set(...at); parent.add(item); return item; }
  function plant(parent, at, scale = 1) {
  ...
  function batch(group) {
    ...
    const buckets = new Map(), inverse = group.matrixWorld.clone().invert();
    group.traverse(item => {

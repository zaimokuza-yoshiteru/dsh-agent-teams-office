...
function lettering(p, text, x, y, color = C.cream, scale = 1) {
...
function box(p, x, y, w, h, fill, border = C.ink) {
  p.rect(x, y, w, h, border); p.rect(x + 1, y + 1, w - 2, h - 2, fill);
}
function plant(p, x, y, large = false) {
  const h = large ? 28 : 18;
...
function chair(p, x, y, accent = C.teal) {
...
function makeSharedOffice() {
  ...
  const block = (x, y, w, h) => { for (let yy = y; yy < y + h; yy++) for (let xx = x; xx < x + w; xx++) collision[yy * WIDTH + xx] = 1; };
...
export function avatar(index, direction, frame) {
  ...
  const r = (x, y, w, h, color) => p.rect(x, y + bob, w, h, color);
...
export function makePreview(office) {
